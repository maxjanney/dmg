...
pub fn exec(ins: u8, regs: &mut Registers, mem: &mut Memory) -> u32 {
    ...
    match ins {
        ...
        0x0d => dec_r!(c),                                                      // DEC C
        0x0e => ld_rn!(c),                                                      // LD C, n
        0x0f => rrca!(regs.a),                                                  // RRCA
        
        0x10 => { regs.stopped = true; 4 }                                      // STOP
        ...
        0x3d => dec_r!(a),                                                      // DEC A
        0x3e => ld_rn!(a),                                                      // LD A, n
        0x3f => { regs.toggle(Flag::C); regs.remove(Flag::N | Flag::H); 4 }     // CCF
        ...
        0xd3 => panic!("Unsupported Opcode: 0xd3"),                             // NONE
        ...
        0xdb => panic!("Unsupported Opcode: 0xdb"),                             // NONE
        0xdc => call_cc_nn!(regs.contains(Flag::C)),                            // CALL C, nn
        0xdd => panic!("Unsupported Opcode: 0xdd"),                             // NONE
        ...
        0xfe => cp_a!(mem.rb(regs.bump()), 8),                                  // CP A, n
...
fn pop_af(regs: &mut Registers, mem: &mut Memory) -> u32 {
...
fn ld_io_n(regs: &mut Registers, mem: &mut Memory) -> u32 {
    let n = mem.rb(regs.bump()) as u16;
    mem.wb(0xff00 | n, regs.a);
    12
}

fn inc_dhl(regs: &mut Registers, mem: &mut Memory) -> u32 {
    let n = mem.rb(regs.hl());
    let r = n.wrapping_add(1);
    mem.wb(regs.hl(), r);
    regs.set(Flag::Z, r == 0);
    regs.set(Flag::H, (n & 0xf) + 1 > 0xf);
    regs.remove(Flag::N);
    12
}
